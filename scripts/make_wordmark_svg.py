"""
Render "NAG" as an EXTRUDED 3D wordmark rasterized to ASCII, and emit it as an
...
# blank rows above and below the art. 7 pads the panel out to 576x449, which
# renders at 490 wide beside the 370-wide portrait and lands within a few px of
# its height, so the two terminal windows read as a matched pair.
...
TEXT = os.environ.get("WORDMARK_TEXT", "NAG")
...
PAD = 18
TITLEBAR_H = 28
# displayed width in px (the viewBox stays at native grid resolution, so the
# render stays crisp when GitHub scales the <img>). 490 pairs with the 370-wide
# portrait panel.
DISPLAY_W = int(os.environ.get("WORDMARK_WIDTH", 490))
...
def emit(frames, mode, out, dur, reveal):
    ...
    fs = CELL_H * 0.92
    n = len(frames)
    disp_h = canvas_h * DISPLAY_W / canvas_w

    p = [
        f'<svg xmlns="http://www.w3.org/2000/svg" width="{DISPLAY_W}" height="{disp_h:.0f}" '
    ...
    p.append(f'<text x="{canvas_w/2:.0f}" y="{TITLEBAR_H/2 + 4:.0f}" fill="{TITLE_TEXT}" '
             f'font-size="11.5" text-anchor="middle">nag@github: ~$ ./wordmark.sh --3d</text>')
